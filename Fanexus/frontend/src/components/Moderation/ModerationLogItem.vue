  ...
  <tr class="moderation-log-item">
    ...
    <td class="moderation-log-item__cell">
      ...
      <expandable class="moderation-log-item__content">
        <div v-if="details.userEmail">
          Suspended user with email address: {{ details.userEmail }}
        </div>
        <div>
          <tags-list
...
export default {
  ...
  computed: {
    actionDisplay() {
      return this.details.blogPost
        ? 'Blog post deleted'
        : this.details.userEmail
        ? 'User suspended'
        : 'Comment deleted';
    },
    actionItem() {
      return (
        this.details.blogPost || this.details.comment || this.details.userEmail
      );
    },
    isShared() {
      return !!(
