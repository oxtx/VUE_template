...
blogPostRouter.get("/:blogPostId", auth, (req: AuthRequest, res: Response) =>
    sequelize
        .transaction(transaction => {
            ...
            return BlogPost.findByPk(blogPostId).then((blogPost): Promise<any> => {
                if(!blogPost) {
...
blogPostRouter.post(
    ...
    (req: AuthRequest, res: Response): Promise<any> =>
        sequelize
            .transaction(transaction => {
                // TODO check if user has permissions to post to communities, blogs, shares etc
                const user: User = req.user;
                const AliasId = req.currentAlias;
                return Promise.all([
                    user.hasAlias(req.currentAlias, { transaction }),
                    Alias.findByPk(req.currentAlias, { transaction }).then(
                        (alias): Promise<boolean> => {
                            ...
                            return alias.hasMemberOfCommunity(
                                req.body.community,
                                { transaction }
                            );
                        }
                    )
                ]).then(([hasAlias, inCommunity]): Response | Promise<any> => {
                    // console.log([hasAlias, hasBlog, inCommunity]);
                    if (false === hasAlias) {
                        return res.send(
                            ErrorReportService.getEnvError(
                                "User does not have the alias you are trying to create the blogpost with",
                                "blogPost_create_fail_wrongAlias"
                            )
                        );
                    }
                    if (false === inCommunity) {
                        return res.send(
                            ErrorReportService.getEnvError(
                                "User does not belong to community",
                                "communityCreate_fail_wrongAlias"
                            )
                        );
                    }
                    ...
                    return BlogPostController.createBlogPost(
                        {
                            ...
                            reblogOfBlogPost: req.body.reblogOfBlogPost,
                            viewingPrivacyGroups: req.body.viewingPrivacyGroups,
                        ...
                        if (req.body.shareTo) {
                            let shareTo: ShareToInterface = req.body.shareTo;
                            // TODO notify user if shares failed ?
                            const blogPromises: Promise<
                                any
                            >[] = shareTo.blogs.map(blogId =>
                                Blog.findByPk(blogId, { transaction }).then(
                                    (blog: Blog) =>
                                        AuthService.userHasPostPermissionForEntity().then(
                                            hasPostPermission => {
                            ...
                            promise = Promise.all(
                                blogPromises.concat(
                                    shareTo.communities.map(communityId =>
...
blogPostRouter.post(
    ...
    (req: AuthRequest, res: Response): Promise<Response> =>
        sequelize
            ...
            .catch(e => {
                // console.error(e);
                console.log('error.blogposts.325')
                return res
                    .status(StatusCodes.BAD_REQUEST)
                    .send(
